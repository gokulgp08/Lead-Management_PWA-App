...
export async function POST({ request }) {
  const { name, email, phone, status, owner_id, company_id } = await request.json()

  try {
    const response = await fetch(`${process.env.SUPABASE_URL}/functions/v1/create-lead`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${process.env.SUPABASE_ANON_KEY}`,
      },
      body: JSON.stringify({ name, email, phone, status, owner_id, company_id }),
    })
