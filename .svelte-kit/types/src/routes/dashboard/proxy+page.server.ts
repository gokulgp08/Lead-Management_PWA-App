...
export const load = async ({ locals: { supabase, user, company } }: Parameters<PageServerLoad>[0]) => {
  if (!user || !company) {
...
export const actions = {
  delete: async ({ request, locals: { supabase, user, company } }: import('./$types').RequestEvent) => {
    if (!user || !company) {
